...
class LoginService {
    ...
    public function handleUser(array $user_data, array $tokens): void {
        ...
        if (empty($line_uid)) {
            ...
            $this->redirectWithError(__('Unable to retrieve LINE user profile.', 'buygo-hub-for-line'));
            return;
        }
    ...
    public function handleEmailSubmit(): void {
        ...
        if (!isset($_POST['_wpnonce']) ||
            ...
            $this->redirectWithError(__('Security verification failed. Please try again.', 'buygo-hub-for-line'));
            return;
        }

        $temp_key = isset($_POST['temp_key']) ? sanitize_text_field(wp_unslash($_POST['temp_key'])) : '';
        if (empty($temp_key)) {
            $this->redirectWithError(__('Login session expired. Please log in again.', 'buygo-hub-for-line'));
            return;
        }
        ...
        if (empty($email) || !is_email($email)) {
            $this->redirectWithError(__('Please enter a valid email address.', 'buygo-hub-for-line'));
            return;
        }
    ...
    private function loginUser(int $user_id, array $user_data, array $tokens): void {
        $user = get_user_by('ID', $user_id);
        if (!$user) {
            $this->redirectWithError(__('User does not exist.', 'buygo-hub-for-line'));
            return;
        }
