...
class OAuthClient {
    ...
    public function authenticate(string $code): array {
        ...
        if (is_wp_error($response)) {
            throw new \Exception(sprintf(
                ...
                __('LINE API connection failed: %s', 'line-hub'), esc_html($response->get_error_message())
            ));
        }
        ...
        if ($status_code !== 200) {
            throw new \Exception(esc_html($body['error_description'] ?? $body['error'] ?? __('Token exchange failed.', 'line-hub')));
        }
    ...
    public function getProfile(string $access_token): array {
        ...
        if (is_wp_error($response)) {
            throw new \Exception(sprintf(
                /* translators: %s: error message */
                __('LINE Profile API connection failed: %s', 'line-hub'), esc_html($response->get_error_message())
            ));
        }
        ...
        if ($status_code !== 200) {
            throw new \Exception(sprintf(
                ...
                __('LINE Profile API error (HTTP %d).', 'line-hub'), (int) $status_code
            ));
        }
