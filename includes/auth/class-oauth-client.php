...
class OAuthClient {
    ...
    public function authenticate(string $code): array {
        ...
        if (is_wp_error($response)) {
            throw new \Exception(sprintf(
                ...
                __('LINE API connection failed: %s', 'buygo-hub-for-line'), esc_html($response->get_error_message())
            ));
        }
        ...
        if ($status_code !== 200) {
            throw new \Exception(esc_html($body['error_description'] ?? $body['error'] ?? __('Token exchange failed.', 'buygo-hub-for-line')));
        }
    ...
    public function getProfile(string $access_token): array {
        ...
        if (is_wp_error($response)) {
            throw new \Exception(sprintf(
                /* translators: %s: error message */
                __('LINE Profile API connection failed: %s', 'buygo-hub-for-line'), esc_html($response->get_error_message())
            ));
        }
        ...
        if ($status_code !== 200) {
            throw new \Exception(sprintf(
                ...
                __('LINE Profile API error (HTTP %d).', 'buygo-hub-for-line'), (int) $status_code
            ));
        }
