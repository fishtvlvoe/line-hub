...
2. `authenticate(string $code): array`
...
- 使用 `wp_remote_retrieve_response_code()` 檢查狀態碼
  ...
  <verify>
檢查 LINE API endpoints 和核心方法：
```bash
grep -E "(AUTH_ENDPOINT|TOKEN_ENDPOINT|createAuthUrl|authenticate|verifyIdToken|getProfile)" includes/auth/class-oauth-client.php
```
  </verify>
  <done>
OAuthClient 類別完成，可產生授權 URL、交換 token、驗證 ID Token、取得用戶資料
  </done>
</task>
...
4. OAuthClient 的 authenticate/verifyIdToken/getProfile 方法有完整的錯誤處理
5. 兩個類別的 namespace 正確，可被自動載入
</success_criteria>
