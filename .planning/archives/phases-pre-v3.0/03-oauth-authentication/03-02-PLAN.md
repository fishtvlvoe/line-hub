...
must_haves:
  ...
  key_links:
    ...
    - from: "includes/auth/class-auth-callback.php"
      ...
      via: "LoginService::handleUser (placeholder)"
      pattern: "LoginService.*handleUser|TODO.*LoginService"
---
...
3. `processCallback(string $code, string $state): void`
   ...
   **重要（Plan 03-03 整合點）：**
   在 processCallback 最後加入 placeholder 標記：
   ```php
...
**注意：**
- LoginService::handleUser() 在 Plan 03-03 實作
- 此 Task 在 processCallback 結尾留下明確的 placeholder 標記
- 此 Task 只負責 OAuth 流程，用戶處理在 Plan 03-03
  </action>
  <verify>
檢查核心方法和錯誤處理：
```bash
grep -E "(function handleRequest|function initiateAuth|function processCallback|function handleError|登入逾時|TODO.*LoginService)" includes/auth/class-auth-callback.php
```
  </verify>
  <done>
AuthCallback 類別完成，可處理認證發起、回調驗證、錯誤顯示，並留有 LoginService 呼叫的 placeholder 標記
  </done>
</task>
...
6. processCallback 結尾有 LoginService placeholder 標記
</success_criteria>
